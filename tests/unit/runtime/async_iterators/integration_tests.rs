...
mod complete_pipeline_tests {
    ...
    async fn test_interval_stream_processing() {
        ...
        let stream = StreamBuilder::interval(Duration::from_millis(10))
            .take(50); // Limited to avoid long test
    ...
    async fn test_multiple_subscriptions_same_stream() {
        // Test multiple subscriptions to the same stream
...
mod real_world_scenarios {
    ...
    async fn test_event_stream_processing() {
        ...
        let event_counts = Arc::new(Mutex::new(std::collections::HashMap::new()));
        let subscription = {
            let counts_clone = Arc::clone(&event_counts);
            stream.subscribe(
                move |event| {
    ...
    async fn test_log_processing_simulation() {
        ...
        let processed_logs = Arc::new(Mutex::new(Vec::new()));
        let error_count = Arc::new(AtomicUsize::new(0));

        let subscription = {
            let logs_clone = Arc::clone(&processed_logs);
...
mod interoperability_tests {
    ...
    async fn test_multiple_stream_composition() {
        ...
        let combined_results = Arc::new(Mutex::new(Vec::new()));

        let subscription1 = {
            let results_clone = Arc::clone(&combined_results);
            stream1.subscribe(
    ...
    async fn test_error_handling_integration() {
        ...
        let subscription = stream.subscribe(
            |value| results.lock().unwrap().push(value),
...
mod complete_pipeline_tests {
    ...
    async fn test_data_processing_pipeline() {
        ...
        let stream = StreamBuilder::from_iter(raw_data)
            ...
            .filter(|(_, value, _)| *value > 10)
            // Group by category
            .buffer(10)
            .map(|batch| {
                let mut groups: HashMap<String, Vec<(i64, i64)>> = HashMap::new();
                for (id, value, category) in batch {
        ...
        let subscription = {
            ...
            stream.subscribe(
                move |grouped_data| {
                    results_clone.lock().unwrap().push(grouped_data);
                },
                |_| {},
                || {},
            )
    ...
    async fn test_realtime_monitoring_pipeline() {
        ...
        let metrics_stream = StreamBuilder::interval(Duration::from_millis(10))
            ...
            .buffer(10)
            .map(|window: Vec<(f64, f64, Instant)>| {
                let avg_cpu = window.iter().map(|(cpu, _, _)| cpu).sum::<f64>() / window.len() as f64;
                let avg_memory = window.iter().map(|(_, mem, _)| mem).sum::<f64>() / window.len() as f64;
                let latest_timestamp = window.last().unwrap().2;
                (avg_cpu, avg_memory, latest_timestamp)
            })
            // Alert on high CPU usage
            .filter(|(avg_cpu, _, _)| *avg_cpu > 50.0)
        ...
        let subscription = {
            ...
            metrics_stream.subscribe(
                move |(cpu, mem, ts)| {
                    alerts_clone.lock().unwrap().push((cpu, mem, ts));
                },
                |_| {},
                || {},
            )
    ...
    async fn test_event_processing_pipeline() {
        ...
        let stream = StreamBuilder::from_iter(events)
            ...
            .map(|event| {
                ...
                (event, processed_at, event_type.to_string())
            })
            // Filter and route different event types
            .filter(|(_, _, event_type)| event_type != "system_event")
            // Group by event type for batch processing
            .buffer(5)
            .map(|batch| {
                let mut grouped: HashMap<String, Vec<Event>> = HashMap::new();
                for (event, _, event_type) in batch {
                    grouped.entry(event_type).or_insert(Vec::new()).push(event);
                }
                grouped
            })
            // Apply different processing per group
            .map(|mut groups| {
                // Process user actions
                if let Some(user_actions) = groups.get_mut("user_action") {
                    user_actions.retain(|event| {
                        if let Event::UserAction { action, .. } = event {
                            action != "logout" // Filter out logout events
                        } else {
                            false
                        }
                    });
                ...
                // Process errors
                if let Some(errors) = groups.get_mut("error") {
                    for error in errors {
        ...
        let subscription = {
            let results_clone = Arc::clone(&results);
            stream.subscribe(
                move |processed_groups| {
                    results_clone.lock().unwrap().push(processed_groups);
                },
                |_| {},
                || {},
            )
...
mod real_world_scenarios {
    ...
    async fn test_api_rate_limiting_simulation() {
        ...
        let stream = StreamBuilder::from_iter(requests)
            ...
            .buffer(10)
            .map(|batch| {
                let mut endpoint_groups: HashMap<String, Vec<APIRequest>> = HashMap::new();
                for req in batch {
                    endpoint_groups.entry(req.endpoint.clone())
                        .or_insert(Vec::new())
                        .push(req);
                }
                endpoint_groups
            ...
            .map(|groups| {
                for (endpoint, requests) in &groups {
                    println!("Processing {} requests for endpoint {}", requests.len(), endpoint);
                }
                groups
            });
    ...
    async fn test_log_aggregation_system() {
        // Simulate distributed log aggregation
        ...
        let stream = StreamBuilder::from_iter(logs)
            // Filter out DEBUG logs
            .filter(|log| log.level != "DEBUG")
            // Group by service and level
            .buffer(20)
            .map(|batch| {
                let mut service_groups: HashMap<String, HashMap<String, Vec<LogEntry>>> = HashMap::new();

                for log in batch {
                    service_groups.entry(log.service.clone())
            ...
            .map(|mut service_groups| {
                for (service, level_groups) in &mut service_groups {
                    // Keep only last 5 ERROR logs per service
                    if let Some(error_logs) = level_groups.get_mut("ERROR") {
                        if error_logs.len() > 5 {
                            error_logs.sort_by_key(|log| log.timestamp);
                            *error_logs = error_logs.split_off(error_logs.len() - 5);
                        }
                    }
                }
                service_groups
            ...
            .filter(|service_groups| {
                service_groups.values()
                    .flat_map(|level_groups| level_groups.get("ERROR").unwrap_or(&Vec::new()))
                    .count() > 3
            });

        let critical_alerts = Arc::new(AtomicUsize::new(0));
        let subscription = {
            let alerts_clone = Arc::clone(&critical_alerts);
            stream.subscribe(
                move |service_groups| {
                    let error_count = service_groups.values()
                        .flat_map(|level_groups| level_groups.get("ERROR").unwrap_or(&Vec::new()))
                        .count();
                    println!("Critical alert: {} error logs across services", error_count);
        ...
        let alerts_count = critical_alerts.load(Ordering::SeqCst);
        println!("Generated {} critical alerts", alerts_count);

        // Should have generated some critical alerts
    ...
    async fn test_financial_transaction_processing() {
        // Simulate financial transaction processing with fraud detection
        ...
        let stream = StreamBuilder::from_iter(transactions)
            ...
            .map(|mut tx| {
                ...
                tx.suspicious = fraud_score > 0.5;
                (tx, fraud_score)
            })
            // Separate suspicious transactions
            .filter(|(_, fraud_score)| *fraud_score > 0.5)
            // Group by account for pattern analysis
            .buffer(10)
            .map(|batch| {
                let mut account_patterns: HashMap<String, Vec<(Transaction, f64)>> = HashMap::new();

                for (tx, score) in batch {
                    account_patterns.entry(tx.account_from.clone())
                        .or_insert(Vec::new())
                        .push((tx, score));
                }
            ...
            // Detect account-level fraud patterns
            .map(|account_patterns| {
                let mut flagged_accounts = Vec::new();

                for (account, transactions) in account_patterns {
                    let avg_score = transactions.iter()
                        .map(|(_, score)| score)
                        .sum::<f64>() / transactions.len() as f64;

                    let high_value_count = transactions.iter()
                        .filter(|(tx, _)| tx.amount > 5000.0)
                        .count();

                    if avg_score > 0.6 || high_value_count > 3 {
                        flagged_accounts.push(account);
                    }
                }
            ...
            .filter(|flagged_accounts| !flagged_accounts.is_empty());

        let fraud_alerts = Arc::new(AtomicUsize::new(0));
        let subscription = {
            let alerts_clone = Arc::clone(&fraud_alerts);
            stream.subscribe(
                move |flagged_accounts| {
                    println!("Fraud alert: {} accounts flagged", flagged_accounts.len());
                    alerts_clone.fetch_add(1, Ordering::SeqCst);
                },
                |_| {},
                || {},
        ...
        println!("Generated {} fraud alerts", alerts_count);

        // Should have detected some fraudulent patterns
        assert!(alerts_count > 0);
...
mod interoperability_tests {
    ...
    async fn test_stream_to_channel_conversion() {
        ...
        let stream = StreamBuilder::from_iter(0..50)
            .map(|x| x * 2)
            .filter(|&x| x < 50);
        ...
        while results.len() < 25 && timeout_count < 100 { // Expect ~25 values
            match tokio::time::timeout(Duration::from_millis(10), rx.recv()).await {
                Ok(Some(value)) => results.push(value),
                Ok(None) => break,
                Err(_) => timeout_count += 1,
        ...
        // Should have collected filtered values
        assert!(!results.is_empty());
        assert!(results.len() <= 25); // x * 2 < 50, so x < 25

        // All values should be even and < 50
    ...
    #[tokio::test]
    async fn test_multiple_stream_composition() {
        // Create multiple streams and combine them
        let stream1 = StreamBuilder::from_iter(0..10).map(|x| format!("stream1: {}", x));
        let stream2 = StreamBuilder::from_iter(10..20).map(|x| format!("stream2: {}", x));
        let stream3 = StreamBuilder::interval(Duration::from_millis(50))
            .take(5)
            .map(|x| format!("interval: {}", x));

        // Combine streams (in a real implementation, would use merge operator)
        let combined_results = Arc::new(Mutex::new(Vec::new()));
        ...
        // Should have values from all streams
        assert!(results.len() >= 15); // 10 + 10 + 5 minimum

        // Check that we have values from different streams
        let stream1_count = results.iter().filter(|s| s.starts_with("stream1:")).count();
        let stream2_count = results.iter().filter(|s| s.starts_with("stream2:")).count();
        let interval_count = results.iter().filter(|s| s.starts_with("interval:")).count();

        assert_eq!(stream1_count, 10);
        assert_eq!(stream2_count, 10);
        assert!(interval_count >= 3); // May not get all 5 due to timing
    ...
    #[tokio::test]
    async fn test_error_propagation_across_streams() {
        // Test error handling across stream transformations
        let stream = StreamBuilder::from_iter(0..20)
            .map(|x| {
                if x == 10 {
                    // Simulate an error condition
                    Err("Simulated error at value 10")
                } else {
                    Ok(x * 2)
                }
            })
        ...
        println!("Got {} results and {} errors", final_results.len(), final_errors.len());

        // Should have processed values before and after the error
        assert!(final_results.len() >= 10); // Values 0-9 (doubled)
...
<parameter name="filePath">c:\Users\cristian.naranjo\Downloads\Vela\tests\unit\runtime\async_iterators\integration_tests.rs