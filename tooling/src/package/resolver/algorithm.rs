...
pub fn apply_version_constraints(mut graph: DependencyGraph) -> Result<DependencyGraph, Error> {
    // Collect all constraints for each package first to avoid borrowing issues
    let mut package_constraints = std::collections::HashMap::new();

    for (package_id, _) in &graph.nodes {
        ...
        package_constraints.insert(package_id.clone(), all_constraints);
    }

    // Now apply the constraints
    for (package_id, constraints) in package_constraints {
