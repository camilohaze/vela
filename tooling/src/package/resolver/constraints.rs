        ...
        match input {
            ...
            s if s.starts_with("^") => {
                let version = Version::parse(&s[1..])
                    .map_err(|_| Error::InvalidVersionConstraint { constraint: s.to_string() })?;
            ...
            s if s.contains(" - ") => {
                ...
                if parts.len() == 2 {
                    ...
                    Err(Error::InvalidVersionConstraint { constraint: s.to_string() })
                }
            }
            s if s.contains(".x") => {
            ...
            _ => {
                ...
                let version = Version::parse(input)
                    .map_err(|_| Error::InvalidVersionConstraint { constraint: input.to_string() })?;
