...
use crate::{DIError, DIResult, container::DIContainer};
use std::any::TypeId;
...
macro_rules! impl_injectable {
    ($struct_name:ident, $($field:ident: $field_type:ty),* $(,)?) => {
        impl $crate::resolver::Injectable for $struct_name {
            fn inject(container: &$crate::container::DIContainer) -> $crate::DIResult<Self> {
                Ok(Self {
                    $(
