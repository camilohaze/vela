...
use crate::http::routing::RouteTable;
...
impl HttpServer {
    ...
    {
        self.routes.insert(method, path, crate::http::middleware::FnHandler(handler));
        self
    }

    /// Add middleware
    pub fn middleware<M: crate::http::middleware::Middleware>(mut self, middleware: M) -> Self {
...
async fn handle_request(
    ...
    // Find route
    let method = crate::http::types::Method::from(vela_req.method.clone());
    let (handler, _params) = match routes.find(&method, &vela_req.uri) {
        Some((handler, params)) => (handler, params),
        None => {
            let response = Response::not_found()
                .with_body(crate::http::types::Body::from("Not Found"));
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_resp = Response::internal_server_error()
                .with_body(crate::http::types::Body::from("Internal Server Error"));
...
async fn convert_hyper_request(hyper_req: HyperRequest<Incoming>) -> Request {
    ...
    Request {
        method: crate::http::types::Method::from(parts.method),
        uri: parts.uri.to_string(),
        version: parts.version,
        headers,
        body: crate::http::types::Body::from(body_bytes),
    }
}
