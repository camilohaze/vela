...
pub struct Next<'a> {
    ...
    handler: &'a dyn crate::http::routing::Handler,
    index: usize,
}

impl<'a> Next<'a> {
    /// Create a new Next instance
    pub fn new(middleware: &'a [Box<dyn Middleware>], handler: &'a dyn crate::http::routing::Handler) -> Self {
        Next {
            middleware,
            handler,
            index: 0,
...
pub use crate::http::routing::Handler;

/// Function pointer handler - re-export from routing
pub use crate::http::routing::FnHandler;
...
impl MiddlewareChain {
    ...
    pub async fn execute(&self, req: Request, handler: &dyn crate::http::routing::Handler) -> Result<Response> {
...
impl Middleware for CorsMiddleware {
    async fn handle(&self, req: Request, next: Next<'_>) -> Result<Response> {
        ...
        if req.method == crate::http::types::Method::OPTIONS {
    ...
    async fn handle(&self, req: Request, next: Next<'_>) -> Result<Response> {
        if let Some(auth_header) = req.header(&self.token_header) {
            if auth_header.starts_with("Bearer ") {
                ...
                Ok(Response::new(crate::http::types::StatusCode::new(401).unwrap())
                    .with_body(crate::http::types::Body::from("Invalid token format")))
            }
        } else {
            Ok(Response::new(crate::http::types::StatusCode::new(401).unwrap())
                .with_body(crate::http::types::Body::from("Missing authorization header")))
        }
    }
}
...
mod tests {
    ...
    async fn dummy_handler(_req: Request) -> Result<Response> {
        Ok(Response::ok().with_body(crate::http::types::Body::from("Hello, World!")))
    }
    ...
    async fn test_middleware_chain() {
        ...
        let req = Request::new(crate::http::types::Method::GET, "/test");
    ...
    async fn test_auth_middleware_missing_token() {
        ...
        let req = Request::new(crate::http::types::Method::GET, "/protected");
        let handler = FnHandler(dummy_handler);
