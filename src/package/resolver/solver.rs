...
impl Clause {
    ...
    pub fn is_unit(&self, assignments: &HashMap<PackageId, Version>) -> Option<&Literal> {
    ...
    pub fn is_satisfied(&self, assignments: &HashMap<PackageId, Version>) -> bool {
        for literal in &self.literals {
            if let Some(assigned_version) = assignments.get(&literal.package) {
                let satisfied = if literal.negated {
                    assigned_version != &literal.version
                } else {
                    assigned_version == &literal.version
...
impl SATSolver {
            ...
            for clause in &self.clauses {
                if let Some(unit_literal) = clause.is_unit(&self.assignments) {
                    ...
                    let version = if unit_literal.negated {
                        ...
                        return Err(Error::SolverError("Complex unit propagation not implemented".to_string()));
                    } else {
                        unit_literal.version.clone()
                    };

                    self.assignments.insert(unit_literal.package.clone(), version);
        ...
        for clause in &self.clauses {
            for literal in &clause.literals {
                if !self.assignments.contains_key(&literal.package) {
                    // Choose this version
                    self.assignments.insert(literal.package.clone(), literal.version.clone());
                    self.implication_graph.insert(
    ...
    fn analyze_conflict(&self, conflict_clause: Clause) -> Clause {
        ...
        for (package, _) in &self.assignments {
            ...
            if let Some(current_version) = self.assignments.get(package) {
