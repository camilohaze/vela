# TASK-004/006: Implementación del Lexer con State Machine y Position Tracking

## ⚠️ IMPORTANTE: Código Python del Compilador

Este documento describe el **lexer implementado en Python** (parte del compilador de Vela).

- **Código mostrado**: Python (usa `while`, `for`, `class`, etc.)
- **Lenguaje Vela**: Funcional puro (NO tiene `while`, `for`, usa `.map()`, `.filter()`)
...
- **Estado:** Completada ✅
- **Fecha:** 2025-11-30
- **Estimación**: TASK-004 (80h) + TASK-006 (12h) = 92h combinadas
- **Commit:** 66c3a49

## 🎯 Objetivo

Implementar un lexer de producción para el lenguaje Vela con:
...
#### 1. src/lexer/token.py (~462 líneas)
...
#### 2. src/lexer/lexer.py (~547 líneas)
**Implementación del lexer con state machine.**
...
##### next_token() -> Token
Main state machine que determina tipo de token según carácter actual.
...
def identifier(self) -> Token:
    ...
    3. Si match → TokenKind del keyword
    4. Sino → IDENTIFIER
    """
    start_pos = self.position.copy()
    lexeme = ""
...
##### number() -> Token
Reconoce números enteros y floats.
...
def number(self) -> Token:
    ...
    Casos:
    - 42 → NUMBER_LITERAL (int)
    - 3.14 → FLOAT_LITERAL (float)
    - 3. → NUMBER + DOT (no float)
    """
    start_pos = self.position.copy()
    lexeme = ""
...
def string(self) -> Token:
    ...
    Detecta:
    - Interpolation ${} → delega a _string_with_interpolation()
    - String simple → procesa escape sequences
    - Unterminated → ERROR
    """
    start_pos = self.position.copy()
...
def _string_with_interpolation(self, start_pos: Position) -> Token:
    ...
    - Parser procesará las interpolaciones
...
def comment_block(self) -> None:
    ...
    # Return ERROR será manejado por next_token()
```
...
#### 3. src/lexer/__init__.py (~15 líneas)
**Module exports.**
...
#### 4. docs/architecture/ADR-004-lexer-state-machine.md (~400 líneas)
**Architecture Decision Record para lexer design.**

Ver archivo completo para detalles. Decisión clave:
...
**Consequences**:
- ✅ Performance O(n) garantizado
- ✅ Control total sobre behavior
- ✅ Debugging directo
- ✅ Flexibilidad para features (interpolation)
...
### Código Generado
- **Total líneas**: ~1,456 líneas (+1 ADR ~400 líneas)
- **Archivos**: 4 (token.py, lexer.py, __init__.py, ADR-004)
- **Commit**: 66c3a49

### Features Implementadas
- ✅ 85+ keywords reconocidos
- ✅ 45+ operators tokenizados
- ✅ Position tracking (line, column, offset)
- ✅ String interpolation con ${}
- ✅ Brace balancing en interpolations
- ✅ Escape sequences (\n, \t, \", \\, \$, etc.)
- ✅ Line comments (//)
- ✅ Block comments (/* */)
- ✅ Error recovery (ERROR tokens)
- ✅ Single-pass O(n) performance

### Performance
- **Complexity**: O(n) donde n = longitud del source
- **Single-pass**: Una sola iteración sobre el source
...
- [x] Literales: números, floats, strings, booleanos
- [x] Comentarios: // y /* */
...
- [x] Tracking automático en advance()
...
/ → comment_line() or comment_block() or DIVIDE
...
- `${users.map(u => u.name)}` → nested braces
- `${getUsers().filter(u => { return u.age > 18 })}` → multiple levels
...
# Unterminated block comment
if is_at_end():
    # next_token() detectará y generará ERROR
    pass
```
...
4. **Error Recovery**: Continue después de error, reporte múltiple
5. **Single-Pass**: O(n) performance garantizado

### Trade-offs

**Pros**:
- ✅ Performance O(n) excelente
- ✅ Control total sobre tokenization
- ✅ Debugging directo (no código generado)
- ✅ Extensible para nuevas features
- ✅ Position tracking automático

**Cons**:
- ❌ ~1,000 líneas de código manual
- ❌ Mantenimiento manual de state machine
- ❌ Cognitive complexity alta en next_token()

### Bugs Encontrados y Corregidos

**PIPE Token Duplicado** (TASK-005):
- **Problema**: PIPE en token.py línea 99 (keyword) y 198 (operator)
- **Fix**: Renamed keyword PIPE → PIPE_KEYWORD
- **Commit**: e4f8308

### Future Improvements

1. **Números hexadecimales**: 0x1A2B
2. **Números binarios**: 0b1010
3. **Underscores en números**: 1_000_000
...
2. **Position tracking crítico**: Essential para error messages útiles en parser
3. **String interpolation compleja**: Brace balancing requiere cuidado especial
4. **Error recovery valioso**: Continue después de error permite reportar múltiples issues
5. **ADRs esenciales**: Documentar decisiones arquitectónicas previene re-discusiones
6. **Tests antes de features**: TDD hubiera evitado bug de PIPE duplicado

---

**TASK-004 + TASK-006 COMPLETADAS** ✅
